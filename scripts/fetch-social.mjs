...
const PER_CANDIDATE_TIMEOUT_MS = 90_000; // 90s hard cap per candidate

const client = new Anthropic({
  maxRetries: 0, // Don't retry on errors — compounds timeouts
});
...
async function fetchActivitySummary(candidate, raceTitle) {
  ...
  const response = await client.messages.create(
    ...
    { timeout: PER_CANDIDATE_TIMEOUT_MS }
  );
...
async function writeStepSummary(rows) {
  ...
  if (!summaryFile) return; // Not running under GitHub Actions

  const lines = [
    '## Activity Summary Run',
    '',
    '| Candidate | Status | Length | Sources | Time |',
    '|---|---|---|---|---|'
  ];

  for (const r of rows) {
    const status = r.status === 'updated' ? '✅ Updated'
      : r.status === 'no-change' ? '🔄 No change'
      : r.status === 'timeout' ? '⏱ Timeout'
      : r.status === 'error' ? '❌ Error'
      : r.status;
    const length = r.summary ? `${r.summary.length} chars` : '—';
    const sources = r.sources != null ? `${r.sources}` : '—';
    const time = `${r.elapsedMs ? Math.round(r.elapsedMs / 1000) : '—'}s`;
    lines.push(`| ${r.name} | ${status} | ${length} | ${sources} | ${time} |`);
  }
...
async function main() {
  ...
  for (const { candidate, raceTitle } of flatCandidates) {
    ...
    try {
      newSummary = await fetchActivitySummary(candidate, raceTitle);
    } catch (err) {
      const isTimeout = err?.name === 'APIConnectionTimeoutError'
        || err?.message?.toLowerCase().includes('timeout');
      status = isTimeout ? 'timeout' : 'error';
      errorMessage = err?.message || String(err);
      console.error(`  ${status === 'timeout' ? '⏱' : '❌'} ${candidate.name}: ${errorMessage}`);

      // Fallback: write a "no activity" entry so the run still produces useful data
      newSummary = {
        date: new Date().toISOString().split('T')[0],
        summary: `No recent public activity was found for ${candidate.name}.`,
        sources: []
      };
    }
    ...
    reportRows.push({
      ...
      sources: newSummary.sources?.length || 0,
  ...
  await writeStepSummary(reportRows);
  console.log('All activity summaries processed.');
}
